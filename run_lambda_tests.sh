#!/bin/bash
# ---------------------------------------------------------
# Script para executar testes do Experimento 2.1 - Lambda
# Coleta métricas de corridas compartilhadas
# ---------------------------------------------------------

BIN="./bin/tp2.out"
INPUT_DIR="./exp2_lambda/inputs"
OUTPUT_DIR="./exp2_lambda/outputs"
METRICS_FILE="./exp2_lambda/metrics_lambda.csv"
...
echo "========================================="
echo "  EXPERIMENT 2.1 - LAMBDA VARIATION"
echo "========================================="
echo ""
...
input_count=$(ls -1 "$INPUT_DIR"/input_lambda_*.txt 2>/dev/null | wc -l)
if [ "$input_count" -eq 0 ]; then
    echo -e "${YELLOW}⚠️  Nenhum arquivo de input encontrado em $INPUT_DIR${NC}"
    echo "Crie arquivos executando: python3 generate_lambda_tests.py"
    exit 1
fi

echo -e "${CYAN}📊 Experimento 2.1: Variação de Lambda${NC}"
...
echo "test_num,lambda,total_demands,individual_rides,shared_rides,pct_shared,avg_passengers_shared,avg_efficiency" > "$METRICS_FILE"
...
for input_file in $(ls -1 "$INPUT_DIR"/input_lambda_*.txt | sort -V); do
    ...
    test_num=$(echo "$filename" | sed 's/input_lambda_\([0-9]*\).txt/\1/')
    
    output_file="$OUTPUT_DIR/output_lambda_${test_num}.txt"
    ...
    if [ $? -eq 0 ]; then
        ...
        lambda=$(head -n 6 "$input_file" | tail -n 1)
        ...
        echo "$test_num,$lambda,$total_demands,$individual_rides,$shared_rides,$pct_shared,$avg_passengers,$avg_efficiency" >> "$METRICS_FILE"
...
echo "========================================="
echo ""
echo -e "${CYAN}📊 Resultados do Experimento 2.1:${NC}"
