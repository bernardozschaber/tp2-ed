#!/bin/bash
# ---------------------------------------------------------
# Script para executar testes do Experimento 1.1 - Delta
# Coleta métricas de corridas compartilhadas
# ---------------------------------------------------------

BIN="./bin/tp2.out"
INPUT_DIR="./exp1_delta/inputs"
OUTPUT_DIR="./exp1_delta/outputs"
METRICS_FILE="./exp1_delta/metrics_delta.csv"
...
echo "========================================="
echo "  EXPERIMENT 1.1 - DELTA VARIATION"
echo "========================================="
echo ""
...
input_count=$(ls -1 "$INPUT_DIR"/input_delta_*.txt 2>/dev/null | wc -l)
if [ "$input_count" -eq 0 ]; then
    echo -e "${YELLOW}⚠️  Nenhum arquivo de input encontrado em $INPUT_DIR${NC}"
    echo "Crie arquivos executando: python3 generate_delta_tests.py"
    exit 1
fi

echo -e "${CYAN}📊 Experimento 1.1: Variação de Delta${NC}"
...
echo "test_num,delta,total_demands,individual_rides,shared_rides,pct_shared,avg_passengers_shared,avg_efficiency" > "$METRICS_FILE"
...
for input_file in $(ls -1 "$INPUT_DIR"/input_delta_*.txt | sort -V); do
    ...
    test_num=$(echo "$filename" | sed 's/input_delta_\([0-9]*\).txt/\1/')
    
    output_file="$OUTPUT_DIR/output_delta_${test_num}.txt"
    ...
    if [ $? -eq 0 ]; then
        ...
        delta=$(head -n 3 "$input_file" | tail -n 1)
        ...
        echo "$test_num,$delta,$total_demands,$individual_rides,$shared_rides,$pct_shared,$avg_passengers,$avg_efficiency" >> "$METRICS_FILE"
...
echo "========================================="
echo ""
echo -e "${CYAN}📊 Resultados do Experimento 1.1:${NC}"
