#!/bin/bash
# ---------------------------------------------------------
# Script para executar testes do Experimento 1.2 - Alpha
# Coleta métricas de corridas compartilhadas
# ---------------------------------------------------------

BIN="./bin/tp2.out"
INPUT_DIR="./exp1_alpha/inputs"
OUTPUT_DIR="./exp1_alpha/outputs"
METRICS_FILE="./exp1_alpha/metrics_alpha.csv"
...
echo "========================================="
echo "  EXPERIMENT 1.2 - ALPHA VARIATION"
echo "========================================="
echo ""
...
input_count=$(ls -1 "$INPUT_DIR"/input_alpha_*.txt 2>/dev/null | wc -l)
if [ "$input_count" -eq 0 ]; then
    echo -e "${YELLOW}⚠️  Nenhum arquivo de input encontrado em $INPUT_DIR${NC}"
    echo "Crie arquivos executando: python3 generate_alpha_tests.py"
    exit 1
fi

echo -e "${CYAN}📊 Experimento 1.2: Variação de Alpha${NC}"
...
echo "test_num,alpha,total_demands,individual_rides,shared_rides,pct_shared,avg_passengers_shared,avg_efficiency" > "$METRICS_FILE"
...
for input_file in $(ls -1 "$INPUT_DIR"/input_alpha_*.txt | sort -V); do
    ...
    test_num=$(echo "$filename" | sed 's/input_alpha_\([0-9]*\).txt/\1/')
    
    output_file="$OUTPUT_DIR/output_alpha_${test_num}.txt"
    ...
    if [ $? -eq 0 ]; then
        ...
        alpha=$(head -n 4 "$input_file" | tail -n 1)
        ...
        echo "$test_num,$alpha,$total_demands,$individual_rides,$shared_rides,$pct_shared,$avg_passengers,$avg_efficiency" >> "$METRICS_FILE"
...
echo "========================================="
echo ""
echo -e "${CYAN}📊 Resultados do Experimento 1.2:${NC}"
