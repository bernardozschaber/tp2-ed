#!/usr/bin/env Rscript
# Experiment 1.2: Relationship between alpha and shared rides percentage
# Author: TP2 Analysis
# Description: Plots how alpha (origin distance threshold) affects ride sharing
...
# Read the CSV file
csv_path <- "./exp1_alpha/metrics_alpha.csv"

# Check if file exists
if (!file.exists(csv_path)) {
  cat("❌ ERRO: Arquivo não encontrado:", csv_path, "\n")
  cat("Execute primeiro: ./run_alpha_tests.sh\n")
  quit(status = 1)
}
...
required_cols <- c("alpha", "pct_shared")
...
# Create the plot
plot <- ggplot(data, aes(x = alpha, y = pct_shared)) +
  # Add points
  ...
  # Labels and title
  labs(
    title = "Experimento 1: Impacto de Alpha no Compartilhamento de Corridas",
    subtitle = "Relação entre distância máxima entre origens e percentual de corridas compartilhadas",
    x = "Alpha - Distância Máxima entre Origens (unidades)",
    y = "Corridas Compartilhadas (%)",
    caption = "Configuração: eta=3, delta=30, beta=1500, lambda=0.6, 100 demandas"
  ) +
...
dir.create("./exp1_alpha", showWarnings = FALSE, recursive = TRUE)

# Save the plot to file
output_file <- "./exp1_alpha/experiment_alpha_vs_sharing.png"
ggsave(output_file, 
       plot = plot, 
       width = 10, 
       height = 6, 
...
cat("========== ESTATÍSTICAS DO EXPERIMENTO 1.2 (ALPHA) ==========\n")
cat(sprintf("Total de testes: %d\n", nrow(data)))
cat(sprintf("Alpha variou de: %.1f até %.1f\n", min(data$alpha), max(data$alpha)))
