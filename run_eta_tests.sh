#!/bin/bash
# ---------------------------------------------------------
# Script para executar testes do Experimento 1.4 - Eta
# Coleta métricas de corridas compartilhadas
# ---------------------------------------------------------

BIN="./bin/tp2.out"
INPUT_DIR="./exp1_eta/inputs"
OUTPUT_DIR="./exp1_eta/outputs"
METRICS_FILE="./exp1_eta/metrics_eta.csv"
...
echo "========================================="
echo "  EXPERIMENT 1.4 - ETA VARIATION"
echo "========================================="
echo ""
...
input_count=$(ls -1 "$INPUT_DIR"/input_eta_*.txt 2>/dev/null | wc -l)
if [ "$input_count" -eq 0 ]; then
    echo -e "${YELLOW}⚠️  Nenhum arquivo de input encontrado em $INPUT_DIR${NC}"
    echo "Crie arquivos executando: python3 generate_eta_tests.py"
    exit 1
fi

echo -e "${CYAN}📊 Experimento 1.4: Variação de Eta${NC}"
...
echo "test_num,eta,total_demands,individual_rides,shared_rides,pct_shared,avg_passengers_shared,avg_efficiency" > "$METRICS_FILE"
...
for input_file in $(ls -1 "$INPUT_DIR"/input_eta_*.txt | sort -V); do
    ...
    test_num=$(echo "$filename" | sed 's/input_eta_\([0-9]*\).txt/\1/')
    
    output_file="$OUTPUT_DIR/output_eta_${test_num}.txt"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "$test_num,$eta,$total_demands,$individual_rides,$shared_rides,$pct_shared,$avg_passengers,$avg_efficiency" >> "$METRICS_FILE"
        
        echo -e "${CYAN}  📊 Métricas: $shared_rides/$total_rides compartilhadas ($pct_shared%)${NC}"
        
        success_count=$((success_count + 1))
    else
        echo -e "${RED}  ❌ Erro ao executar o teste${NC}"
...
echo "========================================="
echo ""
echo -e "${CYAN}📊 Resultados do Experimento 1.4:${NC}"
...
if [ -f "$METRICS_FILE" ]; then
    ...
    echo "  $METRICS_FILE"
    echo ""
    echo -e "${CYAN}📋 Métricas coletadas:${NC}"
    cat "$METRICS_FILE"
    echo ""
fi